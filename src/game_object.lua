...
Set `prefix_config.key = false` on your object instead.]]):format(obj.key), obj.set)
    ...
    -------------------------------------------------------------------------------------------------
    ------- API CODE GameObject.Cache
    -------------------------------------------------------------------------------------------------

    SMODS.Cache = SMODS.GameObject:extend {
        obj_table = {},
        obj_buffer = {},
        set = 'Cache',
        required_params = { 'key' },
        ...
        read = function(self, key)
            return copy_table(self.data[key])
        end,
        request = function(self, key, func)
            return self:read(key) or (func and func())
        end,
        clear = function(self)
        ...
        inject = function() end,
        register = function(self)
            SMODS.Cache.super.register(self)
            self:clear()
        end,
    }

    SMODS.enh_cache = SMODS.Cache { 
        key = 'enh',
